...
import { ArrowLeft, Download, Edit2, CheckCircle, FileText, AlertCircle, Image, Save, Trash2, AlertTriangle, X, Link2, ExternalLink, Info } from 'lucide-react';
import AdminLayout from '../components/AdminLayout';
import { ApiClient } from '../lib/api';
import { Submission, Score } from '../types';
import { decodeSubmissionLink, getYoutubeEmbedUrl } from '../lib/utils';
...
export default function AdminSubmissionDetail() {
  ...
  const isLinkSubmission = submission.current_filename.startsWith('LINK_SUB_');
  const linkUrl = isLinkSubmission ? decodeSubmissionLink(submission.current_filename) : '';
  const youtubeEmbedUrl = linkUrl ? getYoutubeEmbedUrl(linkUrl) : '';
      ...
      <div className="space-y-6 font-sans">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-7 space-y-6">
              ...
              <div className="flex gap-3 pt-2">
                {isLinkSubmission ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-sky-600 hover:bg-sky-700 text-white rounded-lg text-sm font-semibold transition shadow-sm"
                  >
                    <ExternalLink className="w-4 h-4" /> Buka Tautan Video Tugas
                  </a>
                ) : (
                  <>
                    <a
            ...
            {/* PREVIEW CONTAINER */}
            <div className="border border-neutral-200 rounded-xl p-5 bg-white shadow-sm space-y-4">
              <h3 className="font-semibold text-neutral-950">Pratinjau Dokumen / Video</h3>
              
              {isLinkSubmission ? (
                youtubeEmbedUrl ? (
                  <div className="space-y-3">
                    <div className="aspect-video w-full rounded-lg overflow-hidden border border-neutral-200 shadow-sm">
                      <iframe
                        src={youtubeEmbedUrl}
                    ...
                    <div className="flex justify-between items-center bg-sky-50 border border-sky-100 p-3 rounded-lg text-xs text-sky-800">
                      <div className="flex gap-1.5 items-center">
                        <Info className="w-4 h-4 text-sky-500 shrink-0" />
                        <span>Siswa mengumpulkan tautan video YouTube. Anda dapat memutar video langsung di atas.</span>
                      </div>
                      <a href={linkUrl} target="_blank" rel="noreferrer" className="underline font-bold text-sky-700 hover:text-sky-900 whitespace-nowrap ml-2">
                        Buka YouTube
                      </a>
                    </div>
                  </div>
                ) : (
                  <div className="border border-sky-200 bg-sky-50/30 rounded-xl p-6 text-center space-y-4">
                    <Link2 className="w-12 h-12 text-sky-500 mx-auto" />
                    <div className="space-y-1">
                      <h4 className="text-sm font-bold text-neutral-900">Tautan Tugas Siswa</h4>
                      <p className="text-xs text-neutral-600 max-w-sm mx-auto">
                        Siswa mengumpulkan tautan video/situs eksternal. Klik tombol di bawah untuk membukanya.
                      </p>
                    </div>
                    <a
                      href={linkUrl}
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-1.5 px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white font-bold text-xs rounded-xl transition"
                    >
